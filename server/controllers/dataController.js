  ...
  try {
    ...
    if (!device) {
      return res
        .status(404)
        .json({
          error: `Device ${deviceId} is not registered. Please register it first.`,
        });
    }
    ...
    const isCalibrated =
      device.attachedMachine && device.attachedMachine.isCalibrated;
    if (!isCalibrated) {
    ...
    if (
      analysis.zeroDriftAdjustment !== null &&
      analysis.zeroDriftAdjustment !== undefined
    ) {
    ...
    if (device.attachedMachine && device.attachedMachine.isCalibrated) {
      ...
      if (device.attachedMachine.needsMaintenance !== currentNeedsMaintenance) {
        await Machine.findByIdAndUpdate(device.attachedMachine._id, {
          needsMaintenance: currentNeedsMaintenance,
        });
...
exports.getDeviceReadings = async (req, res) => {
  try {
    ...
    // Fetch device and attached machine to get baseline
    const device = await Device.findOne({ deviceId }).populate(
      "attachedMachine",
    );
    ...
    const baseline =
      device && device.attachedMachine ? device.attachedMachine.baseline : null;
    const machine =
      device && device.attachedMachine ? device.attachedMachine : null;
...
exports.saveBatchReadings = async (req, res) => {
  try {
    ...
    if (!device) {
      return res
        .status(404)
        .json({ error: `Device ${deviceId} is not registered.` });
    }
    ...
    const isCalibrated =
      device.attachedMachine && device.attachedMachine.isCalibrated;
    const machineBaseline = isCalibrated
      ? device.attachedMachine.baseline
      : null;
    ...
    for (let i = 0; i < total; i++) {
      ...
      const ageVal =
        typeof reading.age === "number" ? reading.age : total - 1 - i;
    ...
    res
      .status(200)
      .json({
        message: `Batch of ${total} readings saved successfully`,
        count: total,
      });
  } catch (error) {
